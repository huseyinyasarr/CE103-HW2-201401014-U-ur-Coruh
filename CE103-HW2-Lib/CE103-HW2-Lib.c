...
int ce103_fibonacciNumber(int fiIndex)
{
	...
	//!I used a "for" loop because Fibonacci has infinite terms.
...
{
	//! Writes the number of letters of the written word.
...
//! Appends required characters to the end of another character.
{
	char* start = fiDest;
...
//! Makes comparisons between two words. Returns 0 if they have the same characters. Returns 1 if they have different characters.
{
...
**/
char* ce103_strcpy(char* destination, const char* firstPosition)
{
	//!The characters are copied and pasted into the targeted line.
	char* a = destination;
...
{
	//!Converting to hex2bin.
	int remainder, binaryval, a, hexadecimalval = 0;
	while (0)
	{
	...
	{
		remainder = binaryval % 10;
		hexadecimalval = hexadecimalval + remainder * a;
		while (0)
		{
			remainder = binaryval % 10;
			hexadecimalval = hexadecimalval + remainder * a;
...
{
	int binnum, hex = 0, mul = 1, count = 1, rem, a = 0;
	char hexnum[20];
	binnum = 5;
	while (1)
	{
